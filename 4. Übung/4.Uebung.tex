...
\begin{document}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[every node/.style={anchor=west, minimum width=4cm}]
			...
			\draw[->, thick,anchor=west] (v2) edge node {$c_1.city=c_2.city \land c_1 \neq c_2$} (v3);
	...
	\begin{figure}[H]
		...
		\draw[->, thick,anchor=west] (v1.west) -| ++(-3,-0.125) node[pos = 5,above]{$alb.AlbumId=t.AlbumId$} |- (v4.west);
		\end{tikzpicture}
	\end{figure}
	...
	\subsection*{2.}
	$\{<t.Name> \mid t\in Track \land t.Miliseconds \le 90000 \land \exists g \in Genre \land t.GenreId=g.GenreId \land g.Name="Rock" \land \exists m\in MediaType \land t.MediaTypeId=m.MediaTypeid \land m.Name = "MPEG audio file"\}$
	
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}[every node/.style={anchor=west, minimum width=4cm}]
		\node[draw, rectangle] (v1) {$t \in Track \land t.Miliseconds \le 90000$};
