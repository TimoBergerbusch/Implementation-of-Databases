	...
	\begin{enumerate}
		\item relational algebra
			\begin{enumerate}[label=(\alph*)]
				\item $\pi_{country}(\sigma_{percentage=100\land Continent='Africa'}(encompasses))$
				\item $\pi_{lake}( riverthrough \bowtie_{river=river1} \rho_{river1\leftarrow river}(\sigma_{Country="F"}(located)))$
				...
				\item $\rho_{CountryWithTheHighestMountain}(\pi_{name}\\
							...
							\bowtie geo\_Mountain \bowtie \rho_{cName\leftarrow name}(Country)
							)$
			\end{enumerate}
		\item SQL queries
			\lstinputlisting[
